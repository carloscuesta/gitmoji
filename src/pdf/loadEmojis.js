...
var loadEmojis = function(callback) {
  if (emojiList == null)
    emojiList = JSON.parse(fs.readFileSync(__root + 'src/data/gitmojis.json'))
      .gitmojis;
  ...
  function checkEmojis(callback) {
    ...
    async.each(
      ...
      function(emoji, callback) {
        fs.access(__root + 'dist/pdf/emojis/' + emoji.name + '.png', function(
          err
        ) {
  ...
  function downloadEmoji(emoji, callback) {
    ...
    request(options, function(err, res) {
      ...
      .on('end', function() {
        ...
        emoji.path = path.resolve(
          __root + 'dist/pdf/emojis/' + emoji.name + '.png'
        );
        callback(null, emoji);
      })
      .pipe(
        fs.createWriteStream(__root + 'dist/pdf/emojis/' + emoji.name + '.png')
      );
  }
};
